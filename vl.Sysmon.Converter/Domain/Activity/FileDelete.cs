...
{
   public class FileDelete : ConvertEntity
   {
      public static IEnumerable<ActivityMonitoringRule> ConvertRules(
         List<SysmonEventFilteringRuleGroupFileDelete> fileCreateRulesGroup)
      {
         if (fileCreateRulesGroup == null || fileCreateRulesGroup.Count == 0)
            return NothingToConvert("FileDelete");
         ...
         try
         {
            Log.Information("Converting rules for FileDelete..");
         ...
         {
            Log.Error(ex, $"Failure to convert rules for FileDelete.");
         }
