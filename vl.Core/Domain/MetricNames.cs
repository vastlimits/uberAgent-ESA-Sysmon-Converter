namespace vl.Core.Domain
{
   public static class MetricNames
   {
      public const string ProcessDnsQuery = "Process:DnsQuery";
      public const string ProcessStartup = "Process:ProcessStartup";
      public const string ProcessStop = "Process:ProcessStop";
      public const string ProcessNetworkTargetPerformance = "Process:NetworkTargetPerformance";
      public const string ApplicationNetworkConnectFailure = "Application:NetworkConnectFailure";
   }
}
