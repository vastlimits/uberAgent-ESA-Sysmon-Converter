...
For a broader real-world smoke test, run the optional corpus validation script. It clones the referenced upstream
repositories to a temporary folder, converts representative SwiftOnSecurity and sysmon-modular configurations,
checks explicit expected output fragments, validates generated query nesting/string-literal shape, and
batch-converts the sysmon-modular directories without committing third-party content to this repository:

```powershell
.\tools\Validate-Corpus.ps1
```

Wazuh's ruleset contains Sysmon decoders and detection rules rather than Sysmon configuration XML. To include that
repository in the corpus scan and verify that no direct `<Sysmon>` / `<EventFiltering>` inputs are present:

```powershell
.\tools\Validate-Corpus.ps1 -IncludeWazuh
```
